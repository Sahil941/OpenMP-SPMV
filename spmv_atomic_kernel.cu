...
__global__ void spmv_atomic(int nnz, int *coord_row, int *coord_col, double *A, double *x, double *y){
	int tid = blockDim.x * blockIdx.x + threadIdx.x;
	int tnum = blockDim.x * gridDim.x;
	int iteration = nnz % tnum ? nnz/tnum + 1: nnz/tnum;

	for (int i = 0; i < iteration; i++){
		int did = tid + i * tnum;
		if (did < nnz){
			double data = A[did];
			int row = coord_row[did];
			int col = coord_col[did];
			double temp = data * x[col];
...
int main(int argc, char *argv[])
{
  ...
  /************************/
	...
	cudaMalloc(&d_coord_row, (sizeof(int) * blockSize));
	cudaMalloc(&d_coord_col, (sizeof(int) * blockSize));
	cudaMalloc(&d_A, (sizeof(double) * blockSize));
	cudaMalloc(&d_x, (sizeof(double) * blockSize));
	cudaMalloc(&d_y, (sizeof(double) * blockSize));

	cudaMemcpy(d_nnz, thread_num, (sizeof(int) * blockSize), cudaMemcpyHostToDevice);
	cudaMemcpy(d_coord_row, rsIndex, (sizeof(int) * blockSize), cudaMemcpyHostToDevice);
	cudaMemcpy(d_coord_col, cIndex, (sizeof(int) * blockSize), cudaMemcpyHostToDevice);
	cudaMemcpy(d_A, val, (sizeof(double) * blockSize), cudaMemcpyHostToDevice);
	cudaMemcpy(d_x, vec, (sizeof(double) * blockSize), cudaMemcpyHostToDevice);
	cudaMemcpy(d_y, res, (sizeof(double) * blockSize), cudaMemcpyHostToDevice);

	spmv_atomic<<<blockSize, M>>>(d_nnz, d_coord_row, d_coord_col, d_A, d_x, d_y);
	
	cudaMemcpy(res, d_y, (sizeof(double) * blockSize), cudaMemcpyDeviceToHost);
